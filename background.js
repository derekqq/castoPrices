...
const checkAvail = {
  ...
  Warszawa:
    "stores/CAPL?filter[ean]={{ean}}&include=clickAndCollect%2Cstock&nearLatLong=52.231958%2C21.006725&page[size]=1000",
      ...
      .then(function (res) {
        ...
        jsonPromise
          .then(function (val) {
            val.data.forEach((d) => {
              if (
                storeList.indexOf(d.id) === -1 &&
                d.attributes.stock.products[0].quantity > 0
              ) {
                storeList.push({
                  id: d.id,
...
function getPrices(tabId, ean, prodName, storeList, sendResponse) {
  ...
  const getPriceForOne = () => {
    if (storeList.length <= 0) {
      // Zwróć listę sklepów z cenami przez sendResponse
      sendResponse({
      ...
      .then(function (res) {
        ...
        jsonPromise
          .then(function (val) {
            ...
            chrome.storage.local.get(["currentProgressCollection"], (item) => {
              let currentProgressCollection =
                item.currentProgressCollection || [];

              if (allStoresCount === processedStores) {
                currentProgressCollection = currentProgressCollection.filter(
                  (item) => item.name !== prodName
                );
              } else {
                let currentProgressItem = currentProgressCollection.find(
                  (i) => i.name === prodName
                );
